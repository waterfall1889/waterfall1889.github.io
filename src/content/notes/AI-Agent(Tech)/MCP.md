...
![](../images/AI-Agent(Tech)/MCP/2026-08-06-17-21-47-image.png)
...
### 5.2 实践中的MCP分层（Layers）

- 数据层（Data layer）：定义了基于 JSON-RPC 的客户端—服务器通信协议，包括生命周期管理，以及核心原语（primitives），例如 工具（tools）、资源（resources）、提示（prompts） 和 通知（notifications）。
- 传输层（Transport layer）：定义了用于支持客户端与服务器之间数据交换的通信机制和通道，包括与具体传输方式相关的 连接建立、消息封装（framing） 以及 授权（authorization）。

MCP 服务器主要向模型提供三类能力：可读取的数据、可调用的工具，以及可复用的提示模板。

- Resources（资源）：类似“文件”的数据，客户端或模型只能读取
- Tools（工具）：可被大模型调用的函数或操作，调用前通常需要用户授权
- Prompts（提示）：预先编写好的提示模板，用于引导用户或模型完成特定任务

**MCP（Model Context Protocol）的基本原理**可以用一句话概括为：

> **通过统一协议，把外部数据、工具和提示以结构化方式提供给大模型，使模型在受控范围内获取上下文并执行操作。**

**简单拆解就是：**

- **MCP Server** 把能力标准化暴露出来（Resources / Tools / Prompts）

- **AI 应用（客户端）** 按 MCP 协议请求这些能力

- **模型** 在需要时读取资源、调用工具、使用提示

- **协议** 负责约束交互方式与权限，保证安全与可复用

### 5.3 FastMCP

FastMCP 是一个用于构建 MCP（Model Context Protocol）应用的标准框架。

FastMCP 帮你把 **外部系统能力**，用 MCP 规定的方式暴露给大模型，让你不用关心底层协议细节。
